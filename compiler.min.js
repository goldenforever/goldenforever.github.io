...
if(!this.inLink&&(cap=this.rules.url.exec(src))){src=src.substring(cap[0].length);text=escape(cap[0]);href=text;out+=this.renderer.link(href,null,text);continue;}
if(!this.inLink&&(cap=this.rules.email.exec(src))){src=src.substring(cap[0].length);text=this.mangle(cap[0]);href=this.mangle('mailto:')+text;out+=this.renderer.link(href,null,text);continue;}
