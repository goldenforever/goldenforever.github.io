...
compiler = open('compiler.min.js', 'r')
root = open('root.min.js', 'r')
css = open('style.css', 'r')
min_css = open('style.min.css', 'w')
some_js = open('some.min.js', 'w')

new_css = cssmin(css.read())
min_css.write(new_css)

compiler_code = compiler.read().replace('$("#this").remove();</script>', '$("#this").remove();</sc\\\'+\\\'ript>')

out = compiler_code + root.read().replace("[0,'compiler.js'],", "").replace("[1,'style.css'],", "") + \
    'var styles = document.createElement("STYLE");' + \
    'styles.appendChild(document.createTextNode("' + new_css.replace('"', '\\"') + '"));' + \
