...
(function() {
    ...
    function valsToHTML(object, args, content) {
        if (object === "comment") {
            ...
            return '<span class="__contents__">['+args[3]+','+args[2]+','+args[1]+','+args[0]+']</span>';
        }
...
function postprocess(str) {
    ...
    var remaining = 4;
    var strs = ['</div><div class="hide">','</div><div class="__appendices__">','</div><div class="__main__">','</div><div class="__intro__">'];

    function recurseDHM(element, strings) {
        if (element == null) return "";
        var pre = "";
        while (strings && element[0].indexOf(strings[strings.length-1]) > -1) {
            pre += strs[strings.length-1];
            strings.pop();
        }
        remaining = strings.length;
        console.log(pre);
        if (element[3].length < 1) {
            return pre + "<li>" + element[0] + "</li>";
        }
        for (var i=0; i<element[3].length; i++)
            str += recurseDHM(element[3][i], strings);
        return pre + '<li>' + element[0] + '<ol>' + str + '</ol></li>';
    }
    ...
    for (var i = 0; i < matches.length; i++) {
        ...
        var post = remaining ? strs.slice(-remaining).join('') : "";
        console.log(post);
        str = str.replace(/<span class="__contents__">.*?<\/span>/, '<ol class="__contents__"><div class="hide">' + contents + post + "</div></ol>");
    }
    /****************************/
