...
matches[i]=[matches[i],matches[i].substring(27,matches[i].length-7).split("~!#!~")];var element;function deep(element){if(element==null)return"";if(element[3].length<1){return"<li>"+element[0]+"</li>";}
var str="";for(var i=0;i<element[3].length;i++)
str+=deep(element[3][i]);return'<li><span class="__headername__">'+element[0]+'</span><ol>'+str+'</ol></li>';}
tree_str=deep(window.header_tree[window.header_tree.length-1]);tree_str=tree_str.substring(4,tree_str.length-5);for(var i=0;i<matches.length;i++)
str=str.replace(/<span class="__contents__">.*?<\/span>/,'<div class="__contents__">'+tree_str+"</div>");str=str.replace(/<\/script>/g,";try{document.currentScript.classList.add('__executed__')}catch(e){[].slice.call(document.getElementsByTagName('script')).pop().classList.add('__executed__')}</script>");return'<div class="container">'+str+'</div>';}
function contextualise(){$('body').css('opacity','0').css('overflow','hidden');$('p:empty').remove();$('._sjs_this, script:not(.__executed__)').each(function(i,x){eval(x.innerHTML);});(function(){var d=new Date();var day=d.getDate();var month=d.getMonth();var year=d.getFullYear();if(day%10==1)day+="st of ";else if(day%10==2)day+="nd of ";else if(day%10==3)day+="rd of ";else day+="th of ";month=["January","February","March","April","May","June","July","August","September","October","November","December"][month]+", ";$('.__date__').html(day+month+year);})()
