...
str=temp;for(var i=0;i<documents.length;i++){$.ajax({global:false,url:documents[i][0],success:function(content,status,xhr){documents[xhr.index][1]=content?content:"";window.xml_count--;},error:function(xhr,errorType,error){window.xml_count--;documents[xhr.index][1]=null;console.log("Error in {add(...)}: "+errorType+", "+error);},xhrFields:{index:i}});}
