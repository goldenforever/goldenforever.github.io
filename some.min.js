...
function preprocess(str){str=str.replace(/\{--(.|\n)*?--}/g,"");var re=/\{[a-zA-Z_]+\(.*?\).*?}/g;var indices=[],match=re.exec(str),count=1,cont=false,i=3,c;while(match){indices[indices.length]=match.index;match=re.exec(str);}
for(var j=indices.length-1;j>=0;j--)
while(count>0){if(indices[j]+i<str.length){c=str.charAt(indices[j]+i);if(!cont){if(c===')')
cont=true;}else{if(c==='{'){count++;}else if(c==='}'){count--;if(count>0)
...
function addListeners(){var navWidths=[];$('nav').each(function(i,x){x=$(x);var totalWidth=0;var c=x.children();c.each(function(i,x){$(x).css('position','absolute').css('top','0').css('left','0');});console.log(x.parent().attr('id'));for(var j=0;j<c.length;j++)totalWidth+=c.eq(j).width();navWidths[i]=totalWidth+parseInt(x.css('padding-left'))+parseInt(x.css('padding-right'));console.log(navWidths[i]);c.each(function(k,x){$(x).css('position','static');});});$('nav').each(function(i,x){x=$(x);function adjustMenu(){if(x.css('display')!="none"&&x.width()<navWidths[i]){x.children('.link-container').css('display','block');x.children('.separator').css('display','none');}else{x.children('.link-container').css('display','inline');x.children('.separator').css('display','inline');}}
$(window).on('resize orientationChanged',adjustMenu);});if(window.location.hash.length>1)changePage(window.location.hash.replace(/^#/,""));$('body').css('opacity','').css('overflow','');}
window.changePage=function(name){if(name){var tag='#'+name.toLowerCase().replace(/%20| /g,'-').replace(/%22|[^a-z\d\-]]/g,'');var query=$(tag);if(query.length){query.css("display","block");query.siblings('.section').css("display","none");query=query.siblings('nav').first().find('.link-container a');query.css('color','');for(var i=0;i<query.length;i++){if(query[i].innerHTML.toLowerCase().replace(/ /g,'-').replace(/[^a-z\d\-]/g,'')===tag.substring(1)){$(query[i]).css('color','#2c8fdb');break;}}
}}};
