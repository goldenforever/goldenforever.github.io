...
cont=true;else if(c==='{')
count++;else if(c==='}'&&--count>0)
str=str.substring(0,indices[j]+i+1)+'<'+count+'>'+str.substring(indices[j]+i+1);i++;}else break;}}
return str;}
...
var text=node[0];var level=node[1];for(var i=tree.length-1;i>=0;i--){if(tree[i][1]<node[1]){history.push(i);if(tree[i][3])return addNode(tree[i][3],node,history);else eval('window.header_tree['+(""+history).replace(/,/g,"][3][")+'][3].push(["'+text.replace(/[^\\]"/g,'\\"')+'",'+level+',['+history+'],[]]);');return;}}
eval('window.header_tree['+(""+history).replace(/,/g,"][3][")+'][3].push(["'+text.replace(/[^\\]"/g,'\\"')+'",'+level+',['+history.concat([eval('window.header_tree['+(""+history).replace(/,/g,"][3][")+'][3].length')])+'],[]]);');return;}
...
var pageCodeAddress=_hd.parentNode.getAttribute("sj").trim();$.get(pageCodeAddress,function(data){retrieveFiles(data);window.defer(function(){var hash=window.location.hash;var obj={};var opt=_hd.parentNode.attributes;for(var i=0;i<opt.length;i++)obj[opt[i].name]=eval("try{"+opt[i].value+"}catch(e){String(opt[i].value)}");$('body').html(generate(window.fileContent,obj));contextualise();addListeners();window.parsed=true;window.defer(function(){$('pre code').each(function(i,block){hljs.highlightBlock(block);});},"window.hljs");},"window.fileContent");});});
