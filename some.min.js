...
+'">'
+(escaped?code:escape(code,true))
+'\n</code></pre>\n';};Renderer.prototype.blockquote=function(quote){return'<blockquote>\n'+quote+'</blockquote>\n';};Renderer.prototype.html=function(html){return html;};Renderer.prototype.heading=function(text,level,raw){var headerCloses=0;var out="";addNode(window.header_tree,[text,level,[0],[]],[]);for(var i=level;i<7;i++){headerCloses+=counter[""+i];counter[""+i]=0;}
counter[""+level]=1;while(headerCloses>0){out+="</div>";headerCloses--;}
...
matches[i]=[matches[i],matches[i].substring(28,matches[i].indexOf(']')).split(",")];console.log(matches);function recurseDHM(element){if(element==null)return"";if(element[3].length<1)return'<li class="__headername__">'+element[0]+'</li>\n';var children="";for(var i=0;i<element[3].length;i++)
...
str=temp;for(var i=0;i<documents.length;i++){$.ajax({context:$(document.head.i),index:i,global:false,url:documents[i][0],success:function(content,status,xhr){documents[xhr.index][1]=content?content:"";window.xml_count--;},error:function(xhr,errorType,error){window.xml_count--;documents[xhr.index][1]=null;console.log("Error in {add(...)}: "+errorType+", "+error);},xhrFields:{index:i}});}
